...
router.post('/add', async (req, res) => {
  try {
    ...
    if (!/^[A-Za-z0-9]+$/.test(couponCode)) {
      return res.redirect(`/demo-custom/car-project/admin/coupon/new?error=Coupon code must not contain special characters or whitespace`);
    }
    ...
    await coupon.save();
    res.redirect(`/demo-custom/car-project/admin/coupon/new?message=Coupon created successfully`);
  } catch (error) {
    res.redirect(`/demo-custom/car-project/admin/coupon/new?error=${error.message}`);
  }
});
...
router.post('/delete', async (req, res) => {
  try {
    ...
    res.redirect(`/demo-custom/car-project/admin/coupon/list?message=Coupon deleted successfully`);
  } catch (error) {
    res.redirect(`/demo-custom/car-project/admin/coupon/list?error=${error.message}`);
  }
});
